...
def main():
    ...
    epsilon_decay_set = [1.0, 0.995, 0.990, 0.980]

    for epsilon_decay in epsilon_decay_set:
        print("Using epsilon_decay : {:.4f}\n".format(epsilon_decay), end="")
        epsilon_decay_score, _ = training(epsilon_decay=epsilon_decay)
        epsilon_decay_scores.append(epsilon_decay_score)
    plot_hyperparameters(epsilon_decay_scores, output_dir, "epsilon_decay_scores.png", epsilon_decay_set, "epsilon_decay")
    pickle.dump(epsilon_decay_scores, open("epsilon_decay_scores.pickle", "wb"))
